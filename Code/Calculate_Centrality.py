...
def find_interaction(x, method='cosine', threshold=0.2, feature=True):
    ...
    interact_list = list()
    interaction = list()  # 记录联系的list, 也就是edge的weight
    ...
    for i in range(0, len(dist)):
        for j in range(i + 1, len(dist)):
            ...
            if dist[i][j] <= threshold:
                ...
                interact_list.append([int(nodes_name[i][2:]) - 1, int(nodes_name[j][2:]) - 1])  # 这里直接输出node的index
                interaction.append(1 - dist[i][j])
    return interact_list, interaction
